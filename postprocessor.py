...
parser.add_argument('--version',action='version', version='Paraview postprocessor 1.6')
...
for i in range(length):
    ...
    if savedata:
        ...
        volumeB = integrateBubbleData.GetCellData().GetArray('Volume').GetValue(0)
        ...
        for j in range(bubbleCellsNr):
            massB = massB + (cellSizes.GetCellData().GetArray('Volume').GetValue(j)**(2/3)) * bubbleData.GetCellData().GetArray('density').GetValue(j)
